...
const homePage = window.location.href.replace(/\/app.*$/, '')
...
function onDocumentKey(event) {
  ...
  closeMenu()
  headerRef.value?.querySelector('[data-menu-toggle]')?.focus()
}
...
<template>
  <header ref="headerRef" class="bg-burgundy-900 border-b-4 border-gold sticky top-0 z-50 shadow-md">
    <nav class="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4 relative" aria-label="Tools">
      ...
      <div class="hidden md:flex items-center gap-1 sm:gap-2">
        <RouterLink
          ...
          :key="tool.to"
          :to="tool.to"
          class="inline-flex items-center min-h-11 px-3 text-sm font-semibold rounded-md transition no-underline"
      ...
      <button
        type="button"
        data-menu-toggle
        class="md:hidden inline-flex items-center justify-center min-h-11 min-w-11 rounded-md text-white/80 hover:text-gold hover:bg-burgundy-800 transition"
        :aria-expanded="menuOpen"
        aria-controls="app-mobile-menu"
        aria-label="Toggle navigation menu"
        @click="toggleMenu"
      >
        <svg v-if="!menuOpen" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
      ...
      >
        <RouterLink
          ...
          :key="tool.to"
          :to="tool.to"
          class="inline-flex items-center min-h-11 px-3 rounded-md text-sm font-semibold transition no-underline"
          :class="isActive(tool.to)
            ? 'bg-gold text-burgundy-900'
